...
if [ -e ~/world/my_prog/mraster/build ]; then
  ...
  if [ -e ~/world/WWW/site/SS/mraster/ ];  then
    make doc-lib
    make doc-examples
    if [ -e doc-lib/autodocs/html/index.html -a -e doc-examples/autodocs/html/index.html ]; then
      rm -rf ~/world/WWW/site/SS/mraster/doc-lib
      rm -rf ~/world/WWW/site/SS/mraster/doc-examples
      mv doc-lib doc-examples/ ~/world/WWW/site/SS/mraster/
      echo "INFO: Documentation deployed:"
      ls -ld ~/world/WWW/site/SS/mraster/doc-*
    else
      echo "ERROR: Could not find generated doxygen documentation!"
    fi
  else
    echo "ERROR: Could not find mraster web directory: ~/world/WWW/site/SS/mraster/"
  fi
else
