...
for f in ../build/color_lut_docs_csCCfractal*.tiff(:r); do echo $f; nf=pics/cs/`basename $f`.png; convert $f.tiff $nf; done
for f in ../build/color_lut_docs_csCCfractal*.tiff(:r); do echo $f; nf=pics/cs/`basename $f`_50.png; convert -resize 50% $f.tiff $nf; done
