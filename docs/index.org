...
* Design goals
   ...
   - *Easy integration with existing code and libraries* \\
     ...
     numbers.  As another example points for drawing function be be provided as coordinate pairs, point objects, complex numbers, tuples, arrays, etc...
   ...
   - *No Explicit Limits* \\
     Image size, channel depth, and count are limited only by available RAM.  
   - *Sophisticated color API via a zero cost, object based pixel store* \\
     *=MRaster=* is an outlier among image libraries in that pixels are objects.  These objects deliver a very rich API for things like color theory
     computations, color schemes, transformations, etc...  And they do so with *zero* overhead in RAM or runtime performance.  In fact, these templated
     objects are one of the primary tools use to optimize *=MRaster=* performance.
   - *Mathematical Use cases*\\
     Images have both real & integer coordinates, so they may be thought of as part of the real plane.  In addition a great many discrete and continue color
     schemes are supported.  Together these features make *=MRaster=* a very friendly environment for mathematically oriented visualization -- stuff like
     drawing fractals like the Mandelbrot set.
...
While I can't know what everyone uses it for, many users have shared with me how they use *=MRaster=*.
 ...
 - Visualizations in HPC (supercomputer) environemtns.  For large 2D fields, *=MRaster=* can be a less complex and more efficient alternative to traditional
   visualization tools.
