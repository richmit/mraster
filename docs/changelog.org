...
:END:
  ...
  - New functionality
    ...
    - New method colorTpl::channelArithFlt2clrChan
    - Added warning to wMean method regarding the vulgarities of floating point arithmetic and casts
    - Added more compilers to CMakeLists.txt
      - NVHPC
      - IntelLLVM
    - Pixel Corners
      - Moved from experimental to production
      - Functions: int2realSideX, int2realSideY, int2realCorner
      - Example: demo_corners.cpp
    - Added mjr::ramCanvasTpl::getPxColorWrap()
    - Added mjr::ramCanvasTpl::getPxColorChanWrap()
    - markAlpha now defaults to true in mjr::ramCanvasTpl::writeTIFFfile()
  - Documentation
